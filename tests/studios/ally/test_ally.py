...
from studios.ally.ally import (
    get_ally_schedule_and_instructorid_map,
...
def test_get_schedule_from_response_soup(
    ...
    mock_soup = BeautifulSoup(load_response_file("crossstreet_7_to_13_apr.html"), "html.parser")
    ...
    assert result.keys() == expected_results.EXPECTED_CROSSSTREET_7_TO_13_APR_SCHEDULE.keys()
    for key in result:
        assert result[key] == expected_results.EXPECTED_CROSSSTREET_7_TO_13_APR_SCHEDULE[key], (
            f"Expected result list {expected_results.EXPECTED_CROSSSTREET_7_TO_13_APR_SCHEDULE[key]} "
...
def test_get_ally_schedule_and_instructorid_map(
    ...
    mock_week_0_request_response.text = load_response_file("crossstreet_8_to_14_apr.html")

    mock_week_1_request_response = mocker.Mock()
    mock_week_1_request_response.text = load_response_file("crossstreet_15_to_21_apr.html")

    mock_week_2_request_response = mocker.Mock()
    mock_week_2_request_response.text = load_response_file("crossstreet_22_to_28_apr.html")
    ...
    # Call the function to test
    schedule, instructorid_map = get_ally_schedule_and_instructorid_map(mock_logger)

    # Assert that the response is as expected
    assert isinstance(schedule, ResultData)
    assert isinstance(schedule.classes, dict)
    assert schedule.classes.keys() == expected_results.EXPECTED_CROSSSTREET_8_TO_28_APR_SCHEDULE.keys()
    for key in schedule.classes:
        assert schedule.classes[key] == expected_results.EXPECTED_CROSSSTREET_8_TO_28_APR_SCHEDULE[key], (
            f"Expected result list {expected_results.EXPECTED_CROSSSTREET_8_TO_28_APR_SCHEDULE[key]} "
            f"does not match actual result list {schedule.classes[key]}."
        )
    assert instructorid_map == expected_results.EXPECTED_CROSSSTREET_8_TO_28_APR_INSTRUCTORID_MAP

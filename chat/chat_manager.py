...
class ChatManager:
  """
  Manages chat data and interactions between the Telegram bot and chats.
  This class handles query data, messages to be edited or deleted, and communication with chats.
  ...
  def update_query_data_select_all_studios(self, chat_id: int) -> None:
    """
    Updates the selected studios in the query data of the specified chat to all be selected.
  ...
  def add_message_id_to_delete(self, chat_id: int, message_id: int) -> None:
    """
    Adds a message ID to the list of messages to delete for the specified chat on the next call to send_prompt.
  ...
  def send_prompt(
    ...
    """
    Sends a message to the chat and optionally schedules it for deletion on the next call to send_prompt.
